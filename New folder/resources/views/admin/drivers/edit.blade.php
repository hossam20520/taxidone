    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.drivers.update", [$driver->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('wallet') ? 'is-invalid' : '' }}" type="number" name="wallet" id="wallet" value="{{ old('wallet', $driver->wallet) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="email" id="email" value="{{ old('email', $driver->email) }}" required>
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $driver->phone) }}" required>
                @if($errors->has('phone'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('delete') ? 'is-invalid' : '' }}" name="delete" id="delete">
                    ...
                    @foreach(App\Models\Driver::DELETE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('delete', $driver->delete) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('confirm') ? 'is-invalid' : '' }}" name="confirm" id="confirm">
                    ...
                    @foreach(App\Models\Driver::CONFIRM_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('confirm', $driver->confirm) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
