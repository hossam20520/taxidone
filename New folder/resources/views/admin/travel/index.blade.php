@extends('layouts.admin')
@section('content')
@can('travel_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.travel.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.travel.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('global.app_csvImport') }}
            </button>
            @include('csvImport.modal', ['model' => 'Travel', 'route' => 'admin.travel.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.travel.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-Travel">
                <thead>
                    <tr>
                        <th width="10">

                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.id') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.travel') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.travel_cost') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.travel_destination_from') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.travel_destnitation_to') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.travel_destance') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.arrival_time') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.arrival_date') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.client') }}
                        </th>
                        <th>
                            {{ trans('cruds.client.fields.email') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.driver') }}
                        </th>
                        <th>
                            {{ trans('cruds.driver.fields.email') }}
                        </th>
                        <th>
                            {{ trans('cruds.travel.fields.travel_status') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    </tr>
                    <tr>
                        <td>
                        </td>
                        <td>
                            <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                        </td>
                        <td>
                            <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                        </td>
                        <td>
                            <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                        </td>
                        <td>
                            <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                        </td>
                        <td>
                            <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                        </td>
                        <td>
                            <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                        </td>
                        <td>
                        </td>
                        <td>
                        </td>
                        <td>
                            <select class="search">
                                <option value>{{ trans('global.all') }}</option>
                                @foreach($clients as $key => $item)
                                    <option value="{{ $item->email }}">{{ $item->email }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                        </td>
                        <td>
                            <select class="search">
                                <option value>{{ trans('global.all') }}</option>
                                @foreach($drivers as $key => $item)
                                    <option value="{{ $item->name }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                        </td>
                        <td>
                            <select class="search" strict="true">
                                <option value>{{ trans('global.all') }}</option>
                                @foreach(App\Models\Travel::TRAVEL_STATUS_SELECT as $key => $item)
                                    <option value="{{ $item }}">{{ $item }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                        </td>
                    </tr>
                </thead>
                <tbody>
                    @foreach($travel as $key => $travel)
                        <tr data-entry-id="{{ $travel->id }}">
                            <td>

                            </td>
                            <td>
                                {{ $travel->id ?? '' }}
                            </td>
                            <td>
                                {{ $travel->travel ?? '' }}
                            </td>
                            <td>
                                {{ $travel->travel_cost ?? '' }}
                            </td>
                            <td>
                                {{ $travel->travel_destination_from ?? '' }}
                            </td>
                            <td>
                                {{ $travel->travel_destnitation_to ?? '' }}
                            </td>
                            <td>
                                {{ $travel->travel_destance ?? '' }}
                            </td>
                            <td>
                                {{ $travel->arrival_time ?? '' }}
                            </td>
                            <td>
                                {{ $travel->arrival_date ?? '' }}
                            </td>
                            <td>
                                {{ $travel->client->email ?? '' }}
                            </td>
                            <td>
                                {{ $travel->client->email ?? '' }}
                            </td>
                            <td>
                                {{ $travel->driver->name ?? '' }}
                            </td>
                            <td>
                                {{ $travel->driver->email ?? '' }}
                            </td>
                            <td>
                                {{ App\Models\Travel::TRAVEL_STATUS_SELECT[$travel->travel_status] ?? '' }}
                            </td>
                            <td>
                                @can('travel_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.travel.show', $travel->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('travel_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.travel.edit', $travel->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('travel_delete')
                                    <form action="{{ route('admin.travel.destroy', $travel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                    </form>
                                @endcan

                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('travel_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.travel.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  let table = $('.datatable-Travel:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
let visibleColumnsIndexes = null;
$('.datatable thead').on('input', '.search', function () {
      let strict = $(this).attr('strict') || false
      let value = strict && this.value ? "^" + this.value + "$" : this.value

      let index = $(this).parent().index()
      if (visibleColumnsIndexes !== null) {
        index = visibleColumnsIndexes[index]
      }

      table
        .column(index)
        .search(value, strict)
        .draw()
  });
table.on('column-visibility.dt', function(e, settings, column, state) {
      visibleColumnsIndexes = []
      table.columns(":visible").every(function(colIdx) {
          visibleColumnsIndexes.push(colIdx);
      });
  })
})

</script>
@endsection